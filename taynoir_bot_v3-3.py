...
class Config:
    ...
    DERIV_WS_PUBLIC    = os.getenv("DERIV_WS_PUBLIC", "wss://ws.binaryws.com/websockets/v3")
    ...
    DERIV_WS_LEGACY    = os.getenv("DERIV_WS_LEGACY", "wss://ws.binaryws.com/websockets/v3")
...
class DerivConn:
    """
    Connexion Deriv compatible avec la nouvelle API PAT/JWT.

    Nouvelle API :
      PAT/JWT + Deriv-App-ID
        -> GET /trading/v1/options/accounts
        -> POST /trading/v1/options/accounts/{account_id}/otp
        -> WebSocket URL authentifiée retournée par Deriv

    Le PAT n'est donc JAMAIS envoyé directement avec {"authorize": token}
    sur le vieux websocket v3. C'était la cause de "The token is invalid"
    lorsque le robot recevait un pat_... issu de la nouvelle API.
    """
    ...
    @staticmethod
    def _is_modern_token(token: str) -> bool:
        # PAT actuel : pat_... ; OAuth access token : ory_at_...
        return token.startswith(("pat_", "ory_at_"))

    @staticmethod
    def _extract_accounts(payload):
        """Accepte plusieurs formes de réponse afin de rester robuste aux évolutions."""
    ...
    def _account_id(account: dict):
        for key in ("account_id", "accountId", "id", "loginid", "login_id"):
    ...
    @staticmethod
    def _account_is_demo(account: dict):
        for key in ("is_demo", "isDemo"):
            if key in account:
        ...
        if value:
            return "demo" in value or "virtual" in value
        return None

    def _discover_account(self, token):
        """Trouve automatiquement le compte Options demo/real lié au PAT."""
        ...
        if explicit:
            self.account_id = explicit
            log.info(f"Deriv compte configuré: {explicit}")
            return explicit
        ...
        try:
            ...
            if r.status_code != 200:
                try:
                    ...
                    body = r.text[:500]
                log.error(
                    f"Deriv comptes HTTP {r.status_code}: {body}"
                )
                return None

            accounts = self._extract_accounts(r.json())
            if not accounts:
                log.error(
                    "Deriv: aucun compte Options retourné. "
                    "Vérifie que le PAT possède le scope 'trade'."
                )
                return None

            wanted_demo = Config.TRADE_MODE != "real"
            exact = []
            unknown = []
            for account in accounts:
                ...
                if not aid:
                    continue
                is_demo = self._account_is_demo(account)
                if is_demo is None:
                    unknown.append(aid)
                elif is_demo == wanted_demo:
                    exact.append(aid)

            selected = exact[0] if exact else (unknown[0] if len(unknown) == 1 else None)
            if not selected:
                log.error(
                    f"Deriv: impossible de choisir automatiquement le compte "
                    f"{Config.TRADE_MODE}. Comptes détectés: "
                    f"{', '.join(self._account_id(a) or '?' for a in accounts)}. "
                    f"Définis DERIV_ACCOUNT_ID_DEMO/REAL dans Railway."
                )
                return None

            self.account_id = selected
            log.info(f"Deriv compte détecté automatiquement: {selected}")
            return selected
    ...
    def _get_authenticated_ws_url(self, token):
        """Obtient l'URL WS à OTP, comme requis par la nouvelle API Deriv."""
    ...
    def connect(self):
        ...
        if not token:
            log.warning("Deriv: pas de token")
            return False

        if self._is_modern_token(token):
            if not Config.DERIV_APP_ID:
                log.error(
                    "Deriv: DERIV_APP_ID manquant. Avec un PAT pat_..., "
                    "il faut l'App ID de la nouvelle application Deriv."
                )
                return False
            if Config.DERIV_APP_ID.isdigit():
                log.error(
                    f"Deriv: DERIV_APP_ID={Config.DERIV_APP_ID} ressemble à un ancien "
                    "App ID legacy. Crée/utilise l'App ID de la nouvelle application."
                )
                return False
            ...
            # Compatibilité legacy uniquement. Un PAT moderne ne passe jamais ici.
            url = f"{Config.DERIV_WS_LEGACY}?app_id={Config.DERIV_APP_ID}"
            auth_via_otp = False
        ...
        try:
            ...
            if auth_via_otp:
                ...
                if resp and not resp.get("error") and resp.get("balance"):
                    self.authorized = True
                    self.balance = float(resp["balance"].get("balance", 0))
                    self.currency = resp["balance"].get("currency", "USD")
                    log.info(
                        f"✅ Deriv PAT/OTP autorisé | Compte: {self.account_id} | "
                        f"Solde: {self.balance} {self.currency}"
                    )
                elif resp and resp.get("error"):
                    log.error(f"Deriv session: {resp['error'].get('message', 'erreur inconnue')}")
            else:
                # Ancienne API : l'ancien flux authorize reste disponible pour
                # les anciens tokens uniquement.
                resp = self._request({"authorize": token}, timeout=10)
                if resp and not resp.get("error"):
                    self.authorized = True
                    self.balance = float(resp["authorize"].get("balance", 0))
                    self.currency = resp["authorize"].get("currency", "USD")
                    log.info(f"✅ Deriv legacy autorisé | Solde: {self.balance} {self.currency}")
                elif resp:
                    log.error(f"Deriv auth legacy: {resp['error'].get('message', 'erreur inconnue')}")
    ...
    def _on_open(self, ws):
        self.connected = True
        log.info("✅ Deriv WebSocket connecté")
    ...
    def open_position(self, pair_id, direction, stake, sl_distance_price, multiplier=None):
        """
        Ouvre un contrat Multiplier. Le nouveau flux Deriv accepte les paramètres
        directement dans buy; les champs de symbole utilisent underlying_symbol.
        """
        if not self.authorized:
        ...
        # Sécurité : ne jamais envoyer une mise nulle ou supérieure à 15% du solde.
        stake = max(1.0, min(float(stake), self.balance * 0.15))
        if stake <= 0:
        ...
        params = {
            ...
            "limit_order": {
                # Filet de sécurité monétaire conservateur. Le trailing/SAR
                # est ensuite géré par PositionManager.
                "stop_loss": round(max(0.01, stake * 0.90), 2)
            },
        }

        # Nouvelle API : buy peut recevoir directement les paramètres du contrat.
        resp = self._request({
            "buy": 1,
            "price": round(stake, 2),
            "parameters": params,
        ...
        if not contract_id:
            return {"status": "error", "reason": "Réponse Deriv sans contract_id"}

        log.info(
            f"✅ Position Deriv ouverte: #{contract_id} {direction} "
            f"{deriv_sym} stake={stake} mult={mult}"
        )
        return {
            "status": "ok",
            "contract_id": contract_id,
            "buy_price": buy.get("buy_price"),
            "entry_spot": buy.get("start_time") or buy.get("entry_spot"),
            "stake": stake,
            "multiplier": mult,
        }

    def get_contract_status(self, contract_id):
        """Interroge l'état actuel d'un contrat ouvert."""
        resp = self._request(
    ...
    def update_sl(self, contract_id, new_sl_loss_amount):
        """Modifie le stop loss monétaire d'un contrat ouvert."""
        resp = self._request({
            "contract_update": 1,
            "contract_id": contract_id,
    ...
    def close_position(self, contract_id):
        """Ferme un contrat au marché pour le SAR."""
        resp = self._request({"sell": contract_id, "price": 0}, timeout=10)
        if not resp or resp.get("error"):
            reason = resp["error"].get("message") if resp and resp.get("error") else "timeout"
